...
data_trans = 3 * np.power(10, 6)    # 上行传输的数据量为2M
config.glo_data = data_trans
bandwith_ES = 30 * np.power(10, 6)  # 每个ES可分配的带宽为40MHz （可能需要再考虑）
...
for t in range(T_run_1):                                                                             # M-IPD在ES1内运动
    ...
    if t == 0:                                                                                       # 初始状态位置进行一次带宽的分配
        t_last = 0                                                                                   # 上次重新分配时间，初始值为0
        M_x_last = 23.0                                                                              # 上次重分配时M-IPDx坐标，初始为M-IPD的初始坐标
        bandwidth_last = np_arr_ES1[0]                                             # 获得上次的带宽分配方案
    ...
    bandwidth_test = np_arr_ES1[t]          # 判断当前时刻M-IPD所需要的带宽是否大于上次所分配的带宽，若是大于，则也需要进行带宽的重新分配
    ...
    if Gamma >= threshold or flag_band == 1:      # 当决策变量的值大于所设定的阈值时需要进行资源的重新分配
        ...
        bandwidth_last = bandwidth_test                                                             # 将当前带宽分配方案赋给bandwidth_last
        T_trans_ES1 = Delay_with_M_IPD(data_trans, bandwidth_last, ES_coordinate_data[0], 10, IPD_coordinate_1, M_x_last)
        ...
        T_trans_ES1 = Delay_with_M_IPD(data_trans, bandwidth_last, ES_coordinate_data[0], 10, IPD_coordinate_1, M_x)
        T_trans_ES2 = Delay(data_trans, np_arr_ES2[t], ES_coordinate_data[1], 12, IPD_coordinate_2)
        Delay_ES[t] = sum(T_trans_ES1) + sum(T_trans_ES2)
        for i in range(10):
            E_trans_ES1 += ((10 ** (power_tx_F / 10)) / 1000) * T_trans_ES1[i]  # F-IPDs的传输能耗和
        ...
        for i in range(12):
            E_trans_ES2 += ((10 ** (power_tx_F / 10)) / 1000) * T_trans_ES2[i]  # F-IPDs的传输能耗和
        E_trans_ES2 += ((10 ** (power_tx_ES / 10)) / 1000) * (data_ES / R_ES_to_SO)
        Energy_ES[t] = E_trans_ES1 + E_trans_ES2 + p_SO_cycle * CPU_cn * 2          # 无重分配能耗
...
for t in range(T_run_1, T):
    ...
    if t == T_run_1:                                                                                # M-IPD在ES2内运动
        ...
        bandwidth_last = np_arr_ES2[T_run_1]                                                         # 获得上次的带宽分配方案
    ...
    flag_band = 0                                                                                   # 判断M-IPD的所需要带宽是否大于上次分配，0为没有大于，1为大于，需要进行重新分配
    bandwidth_test = np_arr_ES2[t]                               # 判断当前时刻M-IPD所需要的带宽是否大于上次所分配的带宽，若是大于，则也需要进行带宽的重新分配
    ...
    if Gamma >= threshold or flag_band == 1:                                                              # 当决策变量的值大于所设定的阈值时需要进行资源的重新分配
        ...
        M_x_last = M_x                                                                         # 将当前M-IPD位置赋给M_x_last
        bandwidth_last = bandwidth_test                                                             # 将当前带宽分配方案赋给bandwidth_last
        T_trans_ES2 = Delay_with_M_IPD(data_trans, bandwidth_last, ES_coordinate_data[1], 12, IPD_coordinate_2, M_x_last)
        ...
        Energy_ES[t] = E_trans_ES2 + E_trans_ES1 + p_SO_cycle * CPU_cn * 2 + 1
    else:                                                                                           # 没有达到所设定的阈值不需要进行带宽的重新分配
        T_trans_ES2 = Delay_with_M_IPD(data_trans, bandwidth_last, ES_coordinate_data[1], 12, IPD_coordinate_2, M_x)
        T_trans_ES1 = Delay(data_trans, np_arr_ES1[t], ES_coordinate_data[0], 10, IPD_coordinate_1)
        Delay_ES[t] = sum(T_trans_ES2) + sum(T_trans_ES1)
        for i in range(12):
            E_trans_ES2 += ((10 ** (power_tx_F / 10)) / 1000) * T_trans_ES2[i]  # F-IPDs的传输能耗和
        E_trans_ES2 += ((10 ** (power_tx_M / 10)) / 1000) * T_trans_ES2[12] + ((10 ** (power_tx_ES / 10)) / 1000) * (data_ES / R_ES_to_SO)
        for i in range(10):
            E_trans_ES1 += ((10 ** (power_tx_F / 10)) / 1000) * T_trans_ES1[i]  # F-IPDs的传输能耗和
        E_trans_ES1 += ((10 ** (power_tx_ES / 10)) / 1000) * (data_ES / R_ES_to_SO)
        Energy_ES[t] = E_trans_ES2 + E_trans_ES1 + p_SO_cycle * CPU_cn * 2          # 无重分配能耗
