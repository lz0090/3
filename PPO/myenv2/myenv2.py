...
class MyEnv2(gym.Env):
    def __init__(self):
        ...
        self.v_M = v_M
        self.allocation = [1] * num_F                    # 初始分配，每个设备都至少分配一个子载波
    ...
    def step(self, action):
        ...
        # 算当前状态下时延和能耗
        T_trans = Delay(data_trans, self.allocation, ES_coordinate, num_F, IPD_coordinate)                             # 用delay函数计算出每个设备此时的数据传输时延
        ...
        T_trans = Delay(data_trans, self.allocation, ES_coordinate, num_F, IPD_coordinate)                          # 用delay函数计算出每个设备此时的数据传输时延，数组最后一个表示M-IPD
        ...
        E_trans += ((10 ** (power_tx_ES / 10)) / 1000) * (data_ES / R_ES_to_SO)                                        # 加上M-IPD的传输能耗和ES传输到SO的能耗
        E_all = E_trans + p_SO_cycle * CPU_cn                                                                          # 加上SO层的计算能耗   '还没有加上重新分配的能耗'
        for i in range(num_F):
