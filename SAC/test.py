...
class SAC:
    ...
    def load_model(self, path, M_x):
        checkpoint = torch.load(path + '_' + str(M_x))
...
env_evaluate.seed(0)
env_evaluate.action_space.seed(0)
random.seed(0)
np.random.seed(0)
env.seed(0)
torch.manual_seed(0)
...
return_list = rl_utils.train_off_policy_agent(env, env_evaluate, agent, num_episodes,
