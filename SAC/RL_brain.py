...
class SAC:
    ...
    def take_action(self, state):  # 输入当前状态 [n_states]
        ...
        state = torch.tensor(np.array(state)[np.newaxis, :], dtype=torch.float).to(self.device)
    ...
    def save_models(self, path, M_x):
        torch.save(self.actor.state_dict(), f'{path}_actor_{M_x}.pth')
        torch.save(self.critic_1.state_dict(), f'{path}_critic_1_{M_x}.pth')
        torch.save(self.critic_2.state_dict(), f'{path}_critic_2_{M_x}.pth')
        torch.save(self.target_critic_1.state_dict(), f'{path}_target_critic_1_{M_x}.pth')
        torch.save(self.target_critic_2.state_dict(), f'{path}_target_critic_2_{M_x}.pth')
        torch.save({'log_alpha': self.log_alpha, 'alpha_optimizer': self.log_alpha_optimizer.state_dict()},
                   f'{path}_alpha_{M_x}.pth')

    def load_models(self, path, M_x):
        self.actor.load_state_dict(torch.load(f'{path}_actor_{M_x}.pth'))
        self.critic_1.load_state_dict(torch.load(f'{path}_critic_1_{M_x}.pth'))
        self.critic_2.load_state_dict(torch.load(f'{path}_critic_2_{M_x}.pth'))
        self.target_critic_1.load_state_dict(torch.load(f'{path}_target_critic_1_{M_x}.pth'))
        self.target_critic_2.load_state_dict(torch.load(f'{path}_target_critic_2_{M_x}.pth'))
        alpha_data = torch.load(f'{path}_alpha_{M_x}.pth')
        self.log_alpha = alpha_data['log_alpha']
        self.log_alpha_optimizer.load_state_dict(alpha_data['alpha_optimizer'])
