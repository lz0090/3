...
from DE_computing import Delay
import config_MIPD
...
# 
# ###################   可调参数
# num_F = num_IPD_array_1                  # 选择当前仿真的F-IPD数目，num_IPD_array_1代表ES1内的F-IPD数目，num_IPD_array_2代表ES2内的
# ES_coordinate = ES_coordinate_data[0]    # 当前选择的ES，0表示ES1，1表示ES2
# IPD_coordinate = IPD_coordinate_1        # 当前选择的ES内的F-IPD的坐标
# PRIORITY_IPD = num_IPD_array_1           # 定义M-IPD的编号为动作空间中的最后一个取值在这里即为10，要优先给M-IPD进行带宽资源的分配
# EXTRA_num_sub = num_sub_max - (num_IPD_array_1 + 1)  # 因为每个设备都至少需要分配一个子载波，因此先给每个设备都预分配1个子载波，然后算法分配剩余子载波
...
#         self.observation_space = gym.spaces.Box(low=1, high=num_sub_max, shape=((num_IPD_array_1+1),), dtype=np.int32)  # 状态空间取值为每个设备可以分配的载波数，最小为1，最大为最大子载波数50
# 
#     def step(self, action):
# 
#         M_coordinate_x = config_MIPD.glo_M_x  # M-IPD初始x坐标为23,初始y坐标可由AGV的运动轨迹y=x^0.9+1/10*x算出，通过config文件导入变量值
#         reward = 0  # 定义初始奖励值为0
#         T_all = 0   # 定义总时延变量
#         E_all = 0   # 定义总能耗变量
#         E_trans = 0 # 定义传输能耗
...
#                     reward += (T_F_max - T_trans[i])[0] * 20
#                     # reward += 1                                                                                    # 每有一个F-IPD设备时延满足要求，reward加1
#                 else:
#                     reward += (T_F_max - T_trans[i])[0] * 50
...
#         else:
#             next_state = self.allocation  # 下一个状态为执行动作后的设备带宽分配情况
#             reward = -1 if T_trans[num_F] > T_M_max else 1                                                          # 动作值取到为非优先的F-IPD会获得惩罚，且不分配带宽
# 
#         if  flag_M == 1 and flag_F == num_F:
# 
#             reward += ((num_F * T_F_max + T_M_max) / sum(T_trans)) * 400
#             reward -= abs((sum(self.allocation) - num_sub_max)) * 100
#             done = True
# 
# 
#         info = {'T': [sum(T_trans), E_all,self.allocation, sum(self.allocation)]}
# 
# 
# 
...
class MyEnv(gym.Env):
    def __init__(self):
        ...
        self.num_sub_max = num_sub_max
        self.v_M = v_M
        self.allocation = [2] * (num_F + 1)  # 初始分配，每个设备都至少分配一个子载波
        self.allocation[num_F] = config_MIPD.glo_band_M  # M-IPD所分配的带宽

        self.action_space = spaces.Discrete(num_F)   # 离散取值动作空间，动作空间的大小是ES内的设备数目（F-IPD+M-IPD）当前选择ES1
        self.observation_space = gym.spaces.Box(low=1, high=(num_sub_max - config_MIPD.glo_band_M), shape=((num_F+1),), dtype=np.int32)  # 状态空间取值为每个设备可以分配的载波数，最小为1，最大为最大子载波数50
    ...
    def step(self, action):
        ...
        # 算当前状态下时延和能耗
        T_trans_old = Delay(data_trans, self.allocation, ES_coordinate, num_F, IPD_coordinate, M_coordinate_x)                             # 用delay函数计算出每个设备此时的数据传输时延
        ...
        self.allocation[action] += 1                                                                                   # 当M-IPD时延满足要求时只有动作值不取到num_F时才能执行分配动作，为F-IPD分配一个子载波
        next_state = self.allocation                                                                                   # 下一个状态为执行动作后的设备带宽分配情况
        T_trans_new = Delay(data_trans, self.allocation, ES_coordinate, num_F, IPD_coordinate, M_coordinate_x)                          # 用delay函数计算出每个设备此时的数据传输时延，数组最后一个表示M-IPD
    ...
    def reset(self):
        self.allocation = [2] * (num_F + 1)  # 初始分配
        self.allocation[num_F] = config_MIPD.glo_band_M  # M-IPD所分配的带宽
        return np.array(self.allocation)
