...
bandwith_ES = config.glo_band_all  # 每个ES可分配的带宽
...
data_trans = config.glo_data  # 上行传输的数据量为2M
...
class MyEnv(gym.Env):
    def __init__(self):
        ...
        self.allocation[num_F] = config.glo_band_M  # M-IPD所分配的带宽

        self.action_space = spaces.Discrete(num_F)   # 离散取值动作空间，动作空间的大小是ES内的设备数目（F-IPD）当前选择ES1
        self.observation_space = gym.spaces.Box(low=1, high=(num_sub_max - config.glo_band_M), shape=((num_F+1),), dtype=np.int32)  # 状态空间取值为每个设备可以分配的载波数，最小为1，最大为最大子载波数50
