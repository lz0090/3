...
class Agent(object):
    ...
    def save(self, filepath, M_x):
        ...
        torch.save(agent.target_q_net.state_dict(), filepath + '_' + str(M_x) + '_target_q_net.pth')

    def load(self, filepath, M_x):
        agent.q_net.load_state_dict(torch.load(filepath + '_' + str(M_x) + '_q_net.pth'))
        agent.target_q_net.load_state_dict(torch.load(filepath + '_' + str(M_x) + '_target_q_net.pth'))
...
def episode_evaluate(env, agent, M_x):
    ...
    for i in range(5):
        ...
        if tmp_delay_now <= tmp_delay_last and tmp_energy_now <= tmp_energy_last:
            ...
            agent.save('./train_model/M_in_ES2/ES2/DQN_model', M_x)

        reward_list.append(reward_episode)
    return np.mean(reward_list).item(), informa
...
if __name__ == "__main__":
    ...
    for t in range(T_run, T):
        tmp_delay_last = 100
        tmp_energy_last = 100
        config.glo_M_x = M_coordinate_x + v_M * t * (50 / T)  # 当前时隙M-IPD的位置
        config.glo_band_M = int(all_M[t])
        ...
        for i in range(10):
            with tqdm(total=int(num_episodes / 10), desc="Iteration %d" % i) as pbar:
                for episode in range(int(num_episodes / 10)):
                    ...
                    if (episode + 1) % 10 == 0:
                        test_reward, informa = episode_evaluate(env, agent, config.glo_M_x)
                        print(informa)
